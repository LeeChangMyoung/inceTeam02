$(function() {
	...
	$("#btnPassCheck").click(function() {
		...
		$.post("passCheck.ajax", data, function(resultData, status, xhr) {		
			if(status == "success") {
				// eval() 함수는 문자열을 자바스크립트로 코드로 실행해 주는 함수 
				var obj = eval(resultData);
				if(obj.result == -1) {
					alert("존재하지 않는 아이디 입니다.");
				} else if(obj.result == 0) {
					alert("비밀번호가 다릅니다.\n비밀번호를 다시 입력하고 비밀번호 확인을 해 주세요");
					$("#oldPass").val("").focus();
	...
	/* 비밀번호 확인 후에 기존 비밀번호가 다시 수정되면 비밀번호 확인 버튼을
	 * 다시 활성화 시켜서 submit 될때 비밀번호 확인을 다시 하도록 한다.
	 **/
